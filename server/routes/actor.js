...
routes.put("/:actorId", isAuth, isAdmin, uploadImage.single("avatar"), validateImage, [body("name").trim().not().isEmpty().withMessage("Name is required."), body("about").trim().not().isEmpty().withMessage("About is required field."), body("gender").trim().not().isEmpty().withMessage("Gender is required.")], actorControllers.updateActor)
