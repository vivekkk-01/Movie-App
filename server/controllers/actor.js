...
exports.createActor = async (req, res, next) => {
    try {
        ...
        if (req.file) {
            ...
            let indexOfSlash = secure_url.lastIndexOf("/", secure_url.lastIndexOf("/") - 1)
            let image_url = secure_url.substring(0, indexOfSlash) + imgOtpimize + secure_url.substring(indexOfSlash);
...
exports.updateActor = async (req, res, next) => {
    try {
        ...
        if (req.file) {
            ...
            const imgOtpimize = "/h_500,w_500,g_face,c_thumb"
            const { secure_url, public_id } = await cloudinary.uploader.upload(filePath)
            const indexOfSlash = secure_url.lastIndexOf("/", secure_url.lastIndexOf("/") - 1)
            const image_url = secure_url.substring(0, indexOfSlash) + imgOtpimize + secure_url.substring(indexOfSlash);
            actor.avatar = { url: image_url, public_id }
            await actor.save()
            unlink(filePath, (err) => {
                if (err) {
