  ...
  try {
    ...
    if (token) {
      return res
        .status(400)
        .json(
          "You can not ask for new OTP within just 1 hour of sending an OTP."
        );
    }
    ...
    return res
      .status(201)
      .json("A new OTP successfully sent to your email address.");
  } catch (error) {
    return res.status(500).json("Something went wrong, please try again.");
  }
};
  ...
  try {
    ...
    if (token) {
      return res
        .status(400)
        .json(
          "You can not ask us to send new email within just 1 hour of sending an email."
        );
    }
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    if (!resetToken)
      return res
        .status(404)
        .json("Token expired, try resetting your password again.");
    ...
    if (isOldPassword)
      return res.status(400).json("Please provide a new password.");
